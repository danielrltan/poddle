...
| `connect` (phone) | same, where a phone can pair (the https site, or `?padtest=1`) and the player has not chosen the AirPod (NOTES 34) | Title **Grab your paddle** · `#pad-pair` card: QR of `/pad.html?k=CODE` · **Your phone is the paddle** · 1 **Point your phone’s camera at the code.** 2 **Tap Start, then hold the phone like a paddle handle.** · **No camera? Open poddleball.com/pad on the phone and type** CODE · first row named **Phone**: waiting **Scan the code with your phone’s camera.** / bad **Signal lost. Wake the phone and keep its Poddle page open.** · footer **Nothing to install.** In place of the title, a `#paddle-seg` switch **Phone | AirPod** (radiogroup, icons, arrow keys; NOTES 37) picks the paddle and is remembered; AirPod shows the Poddle Helper card instead of the QR. The card hides once the phone is connected. With a phone the calibration drawing (`#calart.is-phone`) and the corner 3D view are a phone, and the settings row says **Show phone**. |
| `calibrate1` | `#cal.menu-bg` (`data-screen="calibrate"`; the id stays `cal` because other code and tests look for it) | Header **Calibration** · steps **1 Hold still**, **2 Tip up** · headline **Hold still** · lead **Hold the AirPod like a paddle handle, pointing at the screen.** · badge counts **5 4 3 2 1** · message line is empty except **Waiting for the AirPod** (no data) and **The camera can’t see you. Step into view.** (tracker ready but no face/body, step 1 only) · footer left **Any grip works. The game learns yours.** · footer right `C` **Start over** · with a phone as the paddle: lead **Hold the phone like a paddle handle, top end pointing at the screen.**, step 2 lead **Tip the top end up toward the ceiling.**, message **Waiting for the phone** (ui.js `PADDLE`) |
...
| `hud` | HUD over the court | Lights **AirPod** **Game** **Camera** (dots only when all is well) · in a room: pill **Room** `CODE` (click copies the link; tooltip **Copy link**, none on localhost) · tabs **You**; opponent **Waiting** (+ **B adds a bot** outside a room; in a room the bot comes by itself), **Rookie Bot** · **Club Bot** · **Pro Bot**, **Player 2** (what the near-side player sees) / **Player 1** (what the far-side player sees) (+ **Near side** / **Far side** under both names in a two-human game; **Setting up** under the opponent while they are still calibrating and the server holds the serve) · **Rally** · keys: `M` **Move: Body** (or **Auto**) · `B` **Bot** · `1 2 3` **Level** · `C` **Calibrate** · `R` **Recentre** · `[ ]` **Range** · `V` **AirPod** · `H` **Stats** · `F` **Full screen** · in a room `Q` **Leave**. At 900 px wide and below the lights and the room pill stack in the left corner, the webcam inset shrinks and the scoreboard drops its avatar chips, so nothing covers the room code. |
...
| `serve-prompt` | + `.toast.on` | **Your serve!** (only during play). Other toasts: **Recentred** · **Line up with the ball, then swing** · **Swing at the ball to serve** · **Body: step to move, tilt the AirPod to walk** · **Auto: the game runs, you swing** · **Club Bot** (during play only) · **Can’t add a bot with two players in** · **Range: step 25% of the view to reach the sideline** · **Range: turn 75° to reach the sideline** · **Saved address didn’t answer. Using this Mac.** · **Weak connection. Smoothing it out.** · **AirPod signal lost** · **Reconnecting to the game** (`hud-airpod-lost`) · rooms: **Opponent left** · **Press Q again to leave** · **Link copied** · **Room closed** · **No answer. Try again.** · asking to play (a spectator): **{name} said no** · **No answer from {name}** · **Someone else asked first**. No toast on a miss. |
