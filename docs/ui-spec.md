...
| (no mock) lobby: `create` / `share` / `code` | same | **Create room**: **Public** / **Private** · note **Shows in open rooms** / **Join by code only** · button **Create**. **Your room**: **Room code** (caps) · four boxes · link + **Copy link** (**Copied** for 1.5 s; the row is hidden on localhost) · button **Start**. **Enter code**: four boxes (aria **Code, 1 of 4** to **4 of 4**) · error **Room not found** / **Room is full** / **No free rooms. Try again soon.** / **Couldn’t join** · button **Join** |
| `connect` | `#screen-connect.menu-bg` (only shown while there is no AirPod data; skipped if the stream is already live) | Header **Getting ready** (+ **Back** when in a room) · title **Connect your AirPod** (no lead: the rows say the rest) · rows: **AirPod** waiting: **Take one AirPod out and hold it in your hand.** / bad: **Signal lost. Check the AirPod is still connected to this Mac.** · **Game** waiting: **Finding the game** / bad: **Can’t reach the game. Trying again.** · **Camera** waiting: **Allow the camera when the browser asks.** / ok: **Stand where it can see you.** / bad or off: **No camera. Press M to change how you move.** · state words **Waiting** / **Ready** / **Problem** / **Off** · footer left **Still waiting? Start the AirPod bridge on this Mac.** · footer right `F` **Full screen** |
...
| `hud` | HUD over the court | Lights **AirPod** **Game** **Camera** (dots only when all is well) · in a room: pill **Room** `CODE` (click copies the link; tooltip **Copy link**, none on localhost) · tabs **You**; opponent **Waiting** (+ **B adds a bot** outside a room; in a room the bot comes by itself), **Rookie Bot** · **Club Bot** · **Pro Bot**, **Player 2** (what the near-side player sees) / **Player 1** (what the far-side player sees) (+ **Near side** / **Far side** under both names in a two-human game; **Setting up** under the opponent while they are still calibrating and the server holds the serve) · **Rally** · keys: `M` **Move: Body** (or **Auto**, **Aim**) · `B` **Bot** · `1 2 3` **Level** · `C` **Calibrate** · `R` **Re-center** · `[ ]` **Range** · `V` **AirPod** · `H` **Stats** · `F` **Full screen** · in a room `Q` **Leave**. At 900 px wide and below the lights and the room pill stack in the left corner, the webcam inset shrinks and the scoreboard drops its avatar chips, so nothing covers the room code. |
...
| `serve-prompt` | + `.toast.on` | **Your serve. Swing to hit it.** (only during play). Other toasts: **Re-centered** · **Line up with the ball, then swing** · **Swing through the ball to serve** · **Body: step to move, tilt the AirPod to walk** · **Auto: the game runs, you swing** · **Aim: turn your wrist to move** · **Club Bot** (during play only) · **Can’t add a bot with two players in** · **Range: step 25% of the view to reach the sideline** · **Range: turn 75° to reach the sideline** · **Saved address didn’t answer. Using this Mac.** · **Weak connection. Smoothing it out.** · **AirPod signal lost** · **Reconnecting to the game** (`hud-airpod-lost`) · rooms: **Opponent left** · **Press Q again to leave** · **Link copied** · **Room closed** · **No answer. Try again.** No toast on a miss. |
