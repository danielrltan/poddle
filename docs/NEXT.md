...
### 14g. Already LIVE from commit fab5598, done by hand on the clean tree and mirrored here best-effort. INTEGRATOR: make sure every one of these survives in the final build, finish what the mirror could not apply, and extend them to the new screens.
- COURT wording (14c) is live for the old screens: strings, `?court=CODE` page URL with `?room=` as a silent alias (`qs.get('court') || qs.get('room')`, setUrl deletes `room` and sets `court`), share link, pasted-link regex `/(?:COURT|ROOM)=.../`. test/menu.mjs and test/rooms-e2e.mjs expect the new strings and `court=` in the PAGE url (socket URLs still say `room=`). New screens must follow.
