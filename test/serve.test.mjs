...
stopB(); await wait(700);                                                // let the receiver's last swing window run out
...
await until(() => since(tHit, 'bounce').length, 4000, 'the serve to land');
let hit = since(tHit, 'hit')[0], launch = since(tHit, 'launch')[0], b = since(tHit, 'bounce')[0];
ok(hit && hit.side === 0 && Math.abs(hit.n - 8 / 28) < 1e-6 && hit.kind === 'drive', `served by the swing: ${JSON.stringify(hit)}`);
...
for (const [power, lob, kind] of [[30, 0.8, 'lob'], [13, 0.8, 'dink']]) {
  ...
  await until(() => since(tHit, 'bounce').length, 5000, kind + ' serve to land');
  hit = since(tHit, 'hit')[0]; b = since(tHit, 'bounce')[0];
  ok(hit && hit.kind === kind && b && b.p[2] < -0.5 && b.p[2] > -6.8 && Math.abs(b.p[0]) < 3.15, `${kind} serve: kind=${hit && hit.kind}, lands ${b && b.p.map(v => v.toFixed(2))}`);
  await until(() => since(tHit, 'point').length, 8000, 'the point');
}
