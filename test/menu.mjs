...
if (ONLY.includes('b')) {
const UI_NAMES = 'showScreen showOverlay currentScreen currentOverlay setNames setScore setServe setRally callout toast toastOff watcherNote notesOff confetti confettiOff setStatus setLink setServerAddress calibrationReset calibration setMode toggle setCamera isVisible setStat fullscreen wake onStart onRetry cleanCode onLobby lobbyView lobbyRooms lobbyBusy lobbyLink codeError setRoom titleRoom pointBanner matchResult show playerName askWatch asking onSettings settings setSettings setPaused setSpectator setWatchers onView setView onRematch rematch hold setBot setPaddle padPair onPaddleSwap paddleKind'.split(' ');
...
{ got.length = 0; rest(); const pg = await openB('player', '', { airpod: true }); let m0 = await mark(pg), cs, i;
  ...
  ok(cs.filter(c => c[0] === 'ui.toast').map(c => c[1]).join('|') === '<i>Eve</i> is here to play' && has(cs, 'ui.setNames', o => o.them === 'Evie' && o.themSub === 'Far side' && o.meSub === 'Near side'), `names: one toast "${cs.filter(c => c[0] === 'ui.toast').map(c => c[1])}", scoreboard follows (raw text: ui.js renders it with textContent)`);
