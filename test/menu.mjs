...
{ got.length = 0; rest(); const pg = await openB('player', '', { airpod: true }); let m0 = await mark(pg), cs, i;
  ...
  ok(J(cs.filter(c => c[0] === 'ui.setSettings' && c[1].sens != null).map(c => c[1].sens).slice(0, 3)) === J([5, 6, 5]) && has(cs, 'ui.show', (id, on) => id === 'podwrap' && on === false) && has(cs, 'ui.show', (id, on) => id === 'dev' && on === true) && has(cs, 'ui.toast', t => t === 'Recentred') && !cs.some(c => c[0] === 'ui.toast' && /Range/.test(c[1])),
    `panel rows: sensitivity 4 -> ${cs.filter(c => c[0] === 'ui.setSettings' && c[1].sens != null).map(c => c[1].sens)}, Show AirPod off, Show stats on, Recentre camera; the steps say nothing`);
  ...
  ok(has(cs, 'ui.show', (id, on) => id === 'podwrap' && on === true) && has(cs, 'ui.setSettings', o => o.airpod === true) && has(cs, 'ui.show', (id, on) => id === 'dev' && on === false) && cs.filter(c => c[0] === 'ui.toast' && /^Range/.test(c[1])).length === 2 && has(cs, 'ui.toast', t => t === 'Recentred') && !has(cs, 'ui.setMode') && J(sent('bot')) === J([{ type: 'bot' }, { type: 'bot', level: 2 }]),
...
{ got.length = 0; const pg = await openB('tour'); let cs, i, m0; await h(pg, 'start'); await sleep(400);
  ...
  ok(!sent('leave').length && i.room === 'ORDN' && has(cs, 'ui.toast', t => /Di is the champion!/.test(t)) && !has(cs, 'ui.champion') && sent('tleave').length === 1, `on an ordinary court the champion is a toast, nobody is pulled off (leave ${sent('leave').length}, room ${i.room}, tleave ${sent('tleave').length})`);
