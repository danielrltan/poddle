...
let side = null, cool = 0, swings = 0, hit = null, ballP = [0, 0, 0], ballV = null, bnc = 0; const hits = [];
ws.on('message', raw => { const m = JSON.parse(raw);
  ...
  if (m.type === 'hit') hit = m.side === side ? (hits.push({ at: Date.now(), p: m.p.slice(), land: null, bends: [], kink: 0 }), hits[hits.length - 1]) : null;
  ...
  if (hit && ballV && m.live && m.b === bnc && Date.now() - hit.at < 600) hit.kink = Math.max(hit.kink, Math.hypot(m.v[0] - ballV[0], m.v[2] - ballV[2]));   // sideways/along change in velocity between two packets: gravity never shows here
  ballP = m.p; ballV = m.v; bnc = m.b;
