...
{ const page = await open('spectate=1&view=free&t=2'); const F = () => page.evaluate(() => ({ ...__scene._dbg.free, y: __scene._dbg.camera.position.y, x: __scene._dbg.camera.position.x, touch: __scene._dbg.renderer.domElement.style.touchAction }));
  ...
  ok(hi.pitch === 80 && lo.pitch === 6 && far.dist === 16 && near.dist === 6 && near.y >= 1.2, `clamps hold: pitch ${lo.pitch}..${hi.pitch}, distance ${near.dist}..${far.dist} m at the lowest pitch (the leash: 30 m out down there is the street), eye ${near.y.toFixed(2)} m up`);
  await drag(0, 100); await page.mouse.move(640, 360); await page.mouse.wheel({ deltaY: 9000 }); await settle(page); const up = await F(); await drag(0, -100); const down = await F();
  ok(up.pitch === 31 && up.dist === 30 && down.pitch === 6 && down.dist === 16, `from ${up.pitch} degrees up it reaches ${up.dist} m; dragged back down the leash pulls it in to ${down.dist} m`);
  const z0 = (await F()).tz; await page.mouse.move(640, 360); await page.mouse.down({ button: 'right' }); await page.mouse.move(940, 360, { steps: 6 }); await page.mouse.up({ button: 'right' }); await settle(page); const z1 = await F(); await page.keyboard.press('ArrowRight'); await settle(page); const z2 = await F();
  ok(z0 === 0 && Math.abs(z1.tz) > 1 && z1.yaw === down.yaw && Math.abs(z2.tz) < Math.abs(z1.tz), `right-drag slides the target along the court (tz ${z1.tz.toFixed(2)}), the orbit stays; ArrowRight slides it back (${z2.tz.toFixed(2)})`);
