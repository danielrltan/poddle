...
// lob0.9: a hard scoop (kind 'lob', n 0.9) must burn white; bet>0.2: struck on a bet at the 0.76 cap, re-aimed to 0.2 (stays pale);
// bet: the same bet with no re-aim (after 0.3 s it falls back to the bet's own colour, capped orange). NOTES 70.
...
const sleep = ms => new Promise(r => setTimeout(r, ms)), NS = [0.15, 0.35, 0.55, 0.75, 0.95, 'lob0.9', 'bet>0.2', 'bet'], SPIN = +process.env.SPIN || 0;   // SPIN=0.8: a sliced ball (the spin streaks must still read over the trail)
...
try {
  for (const [vn, q] of Object.entries(VIEWS)) for (const n of NS) {
    ...
    const r = await page.evaluate((row, vn, spin) => {
      const lob = row === 'lob0.9', bet = typeof row === 'string' && row.startsWith('bet'), n = lob ? 0.9 : bet ? 0.76 : row;
      ...
      sc.onEvent({ type: 'hit', side: 1, n, p: p0, kind: lob ? 'lob' : n > 0.76 ? 'smash' : 'drive', spin, bet: bet ? 1 : undefined });
      const until = (vn === "pov" ? 0.5 : 0.55) * T; for (let t = 0; t <= until; t += 1 / 60) { ms += 1000 / 60; sc.updateBall(at(t), vel(t), true, ms); sc.render(ms);
        if (row === 'bet>0.2' && Math.abs(t - 0.1) < 0.5 / 60) sc.onEvent({ type: 'launch', by: 1, n: 0.2 }); }      // the settled swing, 0.1 s on
    ...
    }, n, vn, SPIN);      // n: a power, or one of the named rows
    const file = `${out}trail-${tag}-${vn}-${n}.png`; await page.screenshot({ path: file });
    console.log(`${tag} ${vn.padEnd(9)} n ${String(typeof n === "number" ? n.toFixed(2) : n).padEnd(7)}  hot ${r.hot.toFixed(2)}  trail px ${String(r.px).padStart(5)}  mean |dRGB| ${String(r.dRGB).padStart(5)}  colour rgb(${r.col}) over rgb(${r.bg})  (${r.meshes} mesh)`);
  ...
  const sheet = await browser.newPage(); await sheet.setViewport({ width: NS.length * 640, height: 2 * 630 });
  const img = f => 'data:image/png;base64,' + fs.readFileSync(f).toString('base64');
  await sheet.setContent(`<body style="margin:0;background:#111;display:grid;grid-template-columns:repeat(${NS.length},640px);font:bold 22px system-ui;color:#fff">${tiles.map(t =>
