...
{ const SHOTS = path.join(root, 'test/ui-shots/accounts'), DL = fs.mkdtempSync(path.join(os.tmpdir(), 'pf-dl-')), ID = '0b1e7c52-4d1a-4f3e-9a6b-2c8d5e7f9a10';
  ...
  { // out of order (every level is free to pick): Rookie and Pro beaten, Club not. The rank is the hardest beaten, the Next button and the caption point at the gap, and the gold track stops at Rookie
    ...
    ok(/^(Bronze|Silver|Gold|Platinum|Diamond|Legend)$/.test(o.rank) && /^(trophy|trophies) · \d+ to /.test(o.cap) && /^st-crest is-/.test(o.crest) && J(o.nodes) === J(['is-won', 'is-next', 'is-locked', 'is-won']) && o.lvl === '1' && o.nextIn === '1',
      `Rookie + Pro beaten: a tier from trophies, Next on Club (${J(o)})`);
  ...
  ok(r.crest === 'st-crest is-bronze' && r.rank === 'Bronze' && r.cap === 'Win a match for your first trophies' && r.streak === 'st-streak' && r.n === '0' && J(r.tags) === J(['Start here', 'Beat Rookie first', 'Beat Club first', 'The final boss']) && r.next === 'Next: beat Rookie Matt' && r.nextIn === '0' && r.lvl === '0' && r.hint && r.chips && r.tiles.every(t => t[0] === '0' && t[2] && t[3]),
    `empty state: a Bronze crest with no trophies, Start here on Rookie with the Next button, the people hint, blue zeros with coaching captions (${J(r)})`);
