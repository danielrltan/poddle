...
let now = 0;                              // sim clock. ONE for every room, so a client's clock offset survives the lobby and a change of room
...
function createRoom(code, pub) {
  ...
  const ready = p => p.bot || p.ready || !AUTOBOT;               // a client sends no 'paddle' until it is calibrated: until then that player cannot see the court, so no point is played (test clients may never send one)
  ...
  // Seat a socket (the lobby layer has checked there is a seat). welcome + botinfo go out from here, as they always have.
  function join(ws) {
    ...
    players.push(me); ws.room = room; ws.pl = me;
  ...
  // A human goes: socket closed, 'leave', or their seat is wanted by their own reconnect (again: the new socket sits
  // down in this same room on this same call, so nothing is torn down and nobody is told).
  function leave(me, again) {
    const i = players.indexOf(me); if (i < 0) return;
    players.splice(i, 1); me.ws.room = me.ws.pl = null;
    if (again) return;
    ...
    broadcast({ type: 'left' });                                 // only the human who stayed can hear this
  ...
  function step() {
    ...
    if (!players.length) return;                                 // an empty room costs nothing while it waits to be joined or closed
    ...
    for (const pl of players) paddles[pl.side] = { x: pl.x, y: pl.y, z: pl.z, q: pl.q, bot: pl.bot, wait: ready(pl) ? undefined : true };   // wait: still calibrating (only sent while true)
    ...
    for (const pl of players) {
      ...
      if (pl.ws.every > 1 && tick % pl.ws.every) continue;              // asked for fewer packets (see 'net'): fewer to lose, and every loss stalls a TCP stream
  ...
  const room = { code, pub, join, leave, onMessage, step, humans, idleAt: Date.now(), waitAt: 0 };
  return room;
}
...
const lobbyMsg = () => JSON.stringify({ type: 'lobby', online: wss.clients.size,
  rooms: [...rooms.values()].filter(r => r.pub && r.humans().length === 1).map(r => ({ code: r.code, players: r.humans().length, open: true })) });
...
function seat(ws, r) {
  ...
  if (ws.cid) for (const o of wss.clients) if (o !== ws && o.cid === ws.cid && o.room) { o.room.leave(o.pl, o.room === r); o.terminate(); }
  ...
  if (r.humans().length >= 2) return false;
  lobby.delete(ws);
  if (ws.viaLobby) tell(ws, { type: 'room', code: r.code, public: r.pub });
...
function joinCode(ws, code) {
  const r = typeof code === 'string' ? rooms.get(code.trim().toUpperCase().slice(0, 8)) : null;   // strings only: String() of a deeply nested array overflows the stack
  if (!r) tell(ws, { type: 'joinfail', reason: 'notfound' }); else if (!seat(ws, r)) tell(ws, { type: 'joinfail', reason: 'full' });
}
...
function quick(ws) {
  ...
  for (const r of rooms.values()) { const n = r.humans().length;
    if (r.pub && n < 2 && (!best || n > best.n || n === best.n && r.waitAt < best.r.waitAt)) best = { r, n }; }
...
wss.on('connection', (ws, req) => {
  ...
  ws.cid = q.get('cid') || null; ws.room = ws.pl = null; ws.viaLobby = q.get('lobby') === '1';

  ws.on('message', raw => { try {                              // one process hosts every room: no message, however malformed, may throw out of here
    const m = JSON.parse(raw);
    if (!m || typeof m !== 'object') return;
    if (m.type === 'ping') return tell(ws, { type: 'pong', c: Number.isFinite(m.c) ? m.c : 0, t: now });   // the client times the round trip itself (in the lobby too). c is echoed only as a number
    ...
    if (ws.room) {
      if (m.type === 'leave' && ws.viaLobby) { ws.room.leave(ws.pl); enterLobby(ws); } else ws.room.onMessage(ws.pl, m);
    } else if (lobby.has(ws)) {
      if (m.type === 'quick') quick(ws); else if (m.type === 'create') create(ws, m.public === true); else if (m.type === 'join') joinCode(ws, m.code);
    }
  } catch (err) { if (!(err instanceof SyntaxError)) console.error('bad message:', err.message); } });
  ws.on('close', () => { if (ws.room) ws.room.leave(ws.pl); lobby.delete(ws); lobbyChanged(); });

  if (!ws.viaLobby) { if (!seat(ws, LOCAL)) { ws.send(JSON.stringify({ type: 'full' })); ws.close(); } return; }
  enterLobby(ws);
  if (q.get('room')) joinCode(ws, q.get('room'));              // a reconnect getting its seat back, or a shared link
});
...
setInterval(() => {
  ...
  let k = 0;
  while (acc >= DT && k++ < 4) { acc -= DT; now += DT; LOCAL.step(); for (const r of rooms.values()) r.step(); }
  if (k) { const ms = Date.now();                               // a room nobody has been in for ROOM_TTL goes (its code may be reused)
    for (const r of rooms.values()) if (ms - r.idleAt > ROOM_TTL && !r.humans().length) { rooms.delete(r.code); console.log(`[${r.code}] room closed`); lobbyChanged(); } }
