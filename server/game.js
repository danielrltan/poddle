...
function solve(p, side, n, dir, lob, slice, blk, curl = 0) {   // curl: 1 lets a hard flat drive bend (a struck, settled swing); serves, bets and every prediction pass 0
  ...
  // A lob carries no lift (NOTES 89): what flies like a lob (u 0.5 and up) is a plain parabola at G, however the wrist rolled on the scoop.
...
function createRoom(code, pub, opts = {}) {   // opts (tournaments, docs/COURTS-TOURNEY.md 4.1): { tour: T, kind: 'warm'|'match', only: cid => bool, sideOf: cid => 0|1|null, vsBot, winAt, label: [nameA, nameB], info: { round, next, final }, onResult, onClose }
  ...
  // swing, the legacy no-final client: every one comes through here). The ball is SEALED at contact (NOTES 89): its height and its
  ...
  const aimV = [0, 0, 0];
  // The ease steers only across and along: the height is the ball's own, ballistic from the paddle (NOTES 89), so the landing time is
